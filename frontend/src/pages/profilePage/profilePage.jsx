...
const ProfilePage = () => {
    ...
    let style = {color:'rgb(0, 74, 47)', backgroundColor: 'rgb(18, 224, 148)', margin: '10px'};
    return (
        <div>
            <NavBar />
                    ...
                    {isEditing ? (
                        <div className="userEditContainer">
                            ...
                            {profileInfo.date_of_birth && 
                                ...
                                <input
                                    ...
                                    onChange={(e) => setProfileInfo({ ...profileInfo, date_of_birth: e.target.value })}
                                />
                                </div>
                            }
                            
                            {/* Add more editable fields for first name, last name, email, etc. */}
                            <Button onClick={handleSaveEdit} style={style}>Save</Button>
                            <Button onClick={handleCancelEdit}style={style}>Cancel</Button>
                        ...
                        <>
                            <div className="userInfoContainer">
                                {profileInfo.username && <div className="userInfo"><h2 className="userAttribute">Username:</h2><h2 className="userAttributeValue">{profileInfo.username}</h2></div>}
                                {profileInfo.first_name && <div className="userInfo"><h2 className="userAttribute">First Name:</h2><h2 className="userAttributeValue">{profileInfo.first_name}</h2></div>}
                                {profileInfo.last_name && <div className="userInfo"><h2 className="userAttribute">Surname:</h2><h2 className="userAttributeValue"> {profileInfo.last_name}</h2></div>}
                                {profileInfo.email && <div className="userInfo"><h2 className="userAttribute">Email:</h2><h2 className="userAttributeValue"> {profileInfo.email}</h2></div>}
                                {profileInfo.date_of_birth && <div className="userInfo"><h2 className="userAttribute">Date of Birth:</h2><h2 className="userAttributeValue"> {profileInfo.date_of_birth}</h2></div>}
                            </div>
                            <Button className="profileButtton" onClick={handleEdit} style={style} >Edit</Button>
                            <Button className="profileButtton" onClick={handleLogout} style={style}>Logout</Button>
                            <Button className="profileButtton" onClick={handleDelete}  style={style}>Delete</Button>
                        </>
                    )}
                </div>
            )}
